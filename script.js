...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const saveData = async (collectionName, data) => { if (!isFirebaseInitialized || !db || !currentUserId) { showToast('A ligação à base de dados falhou.', 'error'); return; } try { await addDoc(collection(db, `artifacts/${appId}/users/${currentUserId}/${collectionName}`), { ...data, timestamp: new Date() }); showToast('Dados guardados com sucesso!'); } catch (e) { showToast('Erro ao guardar dados.', 'error'); console.error(e); } };
    const deleteData = async (collectionName, docId) => { if (!isFirebaseInitialized || !db || !currentUserId) { showToast('A ligação à base de dados falhou.', 'error'); return; } try { await deleteDoc(doc(db, `artifacts/${appId}/users/${currentUserId}/${collectionName}`, docId)); showToast('Registo excluído.'); } catch (e) { showToast('Erro ao excluir registo.', 'error'); console.error(e); } };
    const loadUserSettings = async () => { if (!db || !currentUserId) return; const docRef = doc(db, `artifacts/${appId}/users/${currentUserId}/settings/userPreferences`); const docSnap = await getDoc(docRef); if (docSnap.exists()) { userSettings = { ...userSettings, ...docSnap.data() }; } updateSettingsUI(); };
    const saveUserSettings = async () => { const newSettings = { sludge: { warningLow: getNum('settingSludgeWarningLow'), idealLow: getNum('settingSludgeIdealLow'), idealHigh: getNum('settingSludgeIdealHigh') }}; if (!db || !currentUserId) { showToast('Erro: Base de dados não está ligada.', 'error'); return; } const docRef = doc(db, `artifacts/${appId}/users/${currentUserId}/settings/userPreferences`); try { await setDoc(docRef, newSettings, { merge: true }); userSettings = { ...userSettings, ...newSettings }; showToast('Configurações guardadas!'); } catch (e) { showToast('Erro ao guardar configurações.', 'error'); console.error(e); } };
    ...
    // Funções de Dashboard, Gráficos e CSV
    ...
    const displayHistory = (elementId, collectionName, data, headers, dataRenderer, loadFunction) => {
        ...
        historyElement.innerHTML = `
            ...
            <div class="table-container"><table><thead><tr><th>Data</th>${headers.map(h => `<th>${h}</th>`).join('')}<th class="text-right">Ações</th></tr></thead><tbody>
            ${data.map(entry => {
                const noteIconHtml = entry.note ? `<span class="note-icon relative inline-block align-middle ml-1"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><span class="note-tooltip">${entry.note}</span></span>` : '';
                return `<tr>
                    <td>${new Date(entry.timestamp.toDate()).toLocaleString('pt-BR')} ${noteIconHtml}</td>
                    ${dataRenderer(entry)}
                    <td class="text-right space-x-2"><button data-id="${entry.id}" class="load-btn">Carregar</button><button data-id="${entry.id}" class="delete-btn">Excluir</button></td>
                </tr>`;
            }).join('')}
            </tbody></table></div>`;
    ...
    const setupHistoryListener = (collectionName, dataKey, elementId, headers, dataRenderer, loadFunction) => {
        if (!isFirebaseInitialized || !db || !currentUserId) { const el = getEl(elementId); if (el) el.innerHTML = `<p class="text-center text-sm text-slate-500 py-4">Base de dados indisponível.</p>`; return; }
    ...
    const loadAllHistories = () => {
        setupHistoryListener('sludgeAgeCalculations', 'sludge', 'sludgeAgeHistory', ['ISR (dias)'],
            ...
            (entry) => { getEl('aerationTankVolume').value = entry.aerationTankVolume || ''; getEl('aerationTankVSS').value = entry.aerationTankVSS || ''; getEl('discardFlowRate').value = entry.discardFlowRate || ''; getEl('discardVSS').value = entry.discardVSS || ''; getEl('effluentFlowRate').value = entry.effluentFlowRate || ''; getEl('effluentVSS').value = entry.effluentVSS || ''; getEl('sludgeAgeNote').value = entry.note || ''; calculateSludgeAge(); showSection('sludgeAgeSection'); }
        );
        setupHistoryListener('physicalChemicalCalculations', 'phyChem', 'physicalChemicalHistory', ['Efic. Turb. (%)', 'Efic. Cor (%)'],
            ...
            (entry) => { getEl('phyChemInitialTurbidity').value = entry.initialTurbidity || ''; getEl('phyChemFinalTurbidity').value = entry.finalTurbidity || ''; getEl('phyChemInitialColor').value = entry.initialColor || ''; getEl('phyChemFinalColor').value = entry.finalColor || ''; getEl('phyChemIdealDosage').value = entry.idealDosage || ''; getEl('phyChemDosageUnit').value = entry.dosageUnit || 'mg/L'; getEl('phyChemEtaFlowRate').value = entry.etaFlowRate || ''; getEl('phyChemNote').value = entry.note || ''; calculatePhysicalChemical(); showSection('physicalChemicalSection'); }
        );
        setupHistoryListener('organicLoadCalculations', 'organic', 'organicLoadHistory', ['Carga Afluente (kg/dia)', 'Eficiência (%)'],
            ...
            (entry) => { getEl('organicInfluentConcentration').value = entry.influentConcentration || ''; getEl('organicEffluentConcentration').value = entry.effluentConcentration || ''; getEl('organicLoadFlowRate').value = entry.flowRate || ''; getEl('organicLoadNote').value = entry.note || ''; calculateOrganicLoad(); showSection('organicLoadSection'); }
        );
    };
    
    // Funções de inicialização
    const initializeFirebase = () => {
        if (!firebaseConfig || !firebaseConfig.apiKey) { console.warn("Configuração do Firebase não encontrada."); getEl('userIdDisplay').textContent = `DB Offline (Sem Config)`; setSaveButtonsState(false); return; }
        try {
        ...
        } catch (e) { console.error("Erro fatal ao inicializar Firebase SDK:", e); getEl('userIdDisplay').textContent = `DB Offline (Erro)`; setSaveButtonsState(false); }
    };
